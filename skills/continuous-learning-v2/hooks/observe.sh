...
# Layer 1: entrypoint. Interactive and scheduled/remote sessions may continue.
# sdk-ts: Agent SDK sessions can be human-interactive (e.g. via Happy).
# remote_trigger: scheduled/remote-triggered sessions (e.g. weekly routines);
#   these are opt-in for observation so learning can capture their activity.
