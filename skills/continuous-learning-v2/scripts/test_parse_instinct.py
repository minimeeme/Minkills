...
ZERO_CONFIDENCE_INSTINCT_YAML = """\
---
id: zero-conf-instinct
trigger: "when doing something unproven"
confidence: 0
domain: workflow
scope: project
---
...
## Action
Do not trust this pattern yet.
"""

NEGATIVE_CONFIDENCE_INSTINCT_YAML = """\
---
id: negative-conf-instinct
trigger: "when data is corrupted"
confidence: -0.5
domain: workflow
scope: project
---
...
## Action
This should never surface as usable.
"""


def test_export_preserves_zero_confidence(patch_globals, monkeypatch, tmp_path):
    """confidence: 0 must round-trip through export, not silently become 0.5.

    ``if inst.get(key):`` treats 0.0 as falsy and used to drop the whole
    ``confidence:`` line, so re-importing the exported file fell back to
    the 0.5 default and silently inflated the instinct's confidence.
    """
    tree = patch_globals
    project = _make_project(tree)
    monkeypatch.setattr(_mod, "detect_project", lambda: project)

    (project["instincts_personal"] / "zero.yaml").write_text(ZERO_CONFIDENCE_INSTINCT_YAML)

    out_file = tmp_path / "export.yaml"
    args = SimpleNamespace(scope="project", domain=None, min_confidence=None, output=str(out_file))
    ret = _mod.cmd_export(args)
    assert ret == 0

    exported = out_file.read_text()
    assert "confidence: 0" in exported

    reparsed = parse_instinct_file(exported)
    assert reparsed[0]["confidence"] == 0.0


def test_export_min_confidence_zero_excludes_negative(patch_globals, monkeypatch, tmp_path):
    """--min-confidence 0 is a real filter, not a no-op.

    ``if args.min_confidence:`` treated an explicit ``0`` threshold as
    "no filter" because 0.0 is falsy, so instincts with invalid negative
    confidence values were never excluded.
    """
    tree = patch_globals
    project = _make_project(tree)
    monkeypatch.setattr(_mod, "detect_project", lambda: project)

    (project["instincts_personal"] / "neg.yaml").write_text(NEGATIVE_CONFIDENCE_INSTINCT_YAML)
    (project["instincts_personal"] / "normal.yaml").write_text(SAMPLE_INSTINCT_YAML)

    out_file = tmp_path / "export.yaml"
    args = SimpleNamespace(scope="project", domain=None, min_confidence=0.0, output=str(out_file))
    ret = _mod.cmd_export(args)
    assert ret == 0

    exported = out_file.read_text()
    assert "negative-conf-instinct" not in exported
    assert "test-instinct" in exported
...
# ─────────────────────────────────────────────
# _parse_created_date tests
# ─────────────────────────────────────────────

def test_parse_created_date_handles_microseconds_with_z(tmp_path):
    """A ``created`` timestamp with microseconds + ``Z`` (exactly what
    ``datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')``
    produces whenever microsecond != 0) must be read from frontmatter,
    not silently ignored in favor of the file's mtime.
    """
    p = tmp_path / "old.yaml"
    p.write_text(
        "---\n"
        ...
        "confidence: 0.9\n"
        "domain: general\n"
        'created: "2020-01-01T10:23:45.123456Z"\n'
        "---\n\ncontent\n"
    )
    dt = _mod._parse_created_date(p)
    assert (dt.year, dt.month, dt.day) == (2020, 1, 1)


def test_parse_created_date_handles_microseconds_with_offset(tmp_path):
    p = tmp_path / "old.yaml"
    p.write_text(
        "---\n"
        "id: old-instinct\n"
        ...
        "confidence: 0.9\n"
        "domain: general\n"
        'created: "2020-01-01T10:23:45.123456+00:00"\n'
        "---\n\ncontent\n"
    )
    dt = _mod._parse_created_date(p)
    assert (dt.year, dt.month, dt.day) == (2020, 1, 1)
...
def test_parse_created_date_falls_back_to_mtime_when_missing(tmp_path):
    p = tmp_path / "no-date.yaml"
    p.write_text("---\nid: no-date\ntrigger: \"when x\"\n---\n\ncontent\n")
    dt = _mod._parse_created_date(p)
